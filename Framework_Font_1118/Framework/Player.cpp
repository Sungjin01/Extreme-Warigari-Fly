	...
	col(*transform, Vector2(42.0f, 128.0f))
{
	...
	transform->SetPosition(-50.0f, -50.0f);	//Player의 초기 위치 설정

	transform->SetScale(transform->scale.x * 0.7, transform->scale.y * 0.7);
...
void Player::Update()
{
	...
	if (InputManager::GetMyKeyState(VK_UP)&&jumpTime > 0) {
		velocity -= gravity * 3.0 * TimeManager::GetDeltaTime();
		jumpTime -= TimeManager::GetDeltaTime();
	}
	if (InputManager::GetMyKeyState(VK_DOWN)) {
		velocity += gravity * 2.5 * TimeManager::GetDeltaTime();
	}
	if (InputManager::GetKeyDown(VK_SPACE) && jumpTime > 0.4f) {
		velocity -= gravity * 100.0 * TimeManager::GetDeltaTime();
		jumpTime -= 0.4f;
	}
	...
	if (transform->position.y < 70) { //게임 오버~!!!!!!!!!!!!!!!!
		if (velocity < 0) {
			velocity = 0;
		}
	}
